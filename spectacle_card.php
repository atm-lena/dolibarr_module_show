...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	if ($action == 'delete')
	{
	    $formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('Deletespectacle'), $langs->trans('ConfirmDeletespectacle'), 'confirm_delete', '', 0, 1);
	}
	...
	if ($action == 'clone') {
		...
		$formquestion = array();
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('Clonespectacle'), $langs->trans('ConfirmClonespectacle', $object->ref), 'confirm_clone', $formquestion, 'yes', 1);
	}
	...
	/*
	// Ref bis
	$morehtmlref.=$form->editfieldkey("RefBis", 'ref_client', $object->ref_client, $object, $user->rights->modulespectacle->creer, 'string', '', 0, 1);
	$morehtmlref.=$form->editfieldval("RefBis", 'ref_client', $object->ref_client, $object, $user->rights->modulespectacle->creer, 'string', '', null, null, '', 1);
	// Thirdparty
